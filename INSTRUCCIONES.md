...
* **`Entrenando_RF/`:** Incluye los scripts de R con las especificaciones, hiperparámetros y flujo de trabajo (`tidymodels`/`ranger`) utilizados para el entrenamiento del modelo de *Random Forest*. El modelo ajustado resultante se guarda en el archivo **`rf_alquileres_Montevideo`**.

* **`APP_SHINY/`:** Código fuente de la aplicación interactiva construida en Shiny. Permite a los usuarios visualizar el mapa espacial de precios y desplegar el módulo predictivo para estimar el precio medio de alquiler según los insumos ingresados.
